...
[RequireComponent(typeof(VRCamInput))]
public class VRSSD : MonoBehaviour
{
    [SerializeField]
    ...
    private void Start()
    {
        ...
        GetComponent<VRCamInput>().onTextureUpdate.AddListener(Invoke);
    }

    private void OnDestroy()
    {
        GetComponent<VRCamInput>().onTextureUpdate.RemoveListener(Invoke);
        ssd?.Dispose();
    }
    ...
    private void SetFrame(Text frame, SSD.Result result, Vector2 size)
    {
        ...
        Debug.Log("Found object name: " + frame.text);
